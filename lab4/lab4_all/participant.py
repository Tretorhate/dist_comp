...
def main():
    ...
    def wal_append(line: str) -> None:
        if not WAL_PATH:
            return
        # Flushes and syncs to disk to ensure the record survives a crash
        with open(WAL_PATH, "a", encoding="utf-8") as f:
            f.write(line.rstrip("\n") + "\n")
            f.flush()
            os.fsync(f.fileno()) #
        ...
        with open(WAL_PATH, "r") as f:
            for line in f:
                ...
                if len(parts) >= 2:
                    ...
                    # Restore the transaction state from the log
                    TX[txid] = {"state": state, "op": {}} #
        print(f"[{NODE_ID}] Recovered {len(TX)} transactions from WAL.")
